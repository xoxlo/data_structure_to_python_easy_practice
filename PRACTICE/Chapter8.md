...
### 완전이진트리와 포화이진트리의 차이점을 쓰시오.
    완전이진트리는 마지막 레벨 전까지 모든 노드가 다 채워져있고, 마지막 레벨 제일 왼쪽부터 오른쪽으로 노드가 순서대로 채워져 있는 트리다.
    포화이진트리는 각 레벨에 노드가 꽉 차있는 이진트리이다.
----------------------------------------------------------------------------------------------------
### 이진트리의 성질과 부모, 자식 노드를 어떻게 표현하는지 쓰시오.
    n개의 노드를 가진 트리는 n-1개의 간선을 가진다.
    노드 i의 부모 노드는 i/2, 왼쪽 자식 노드는 2i, 오른쪽 자식 노드는 2i+1로 표현
----------------------------------------------------------------------------------------------------
### 이진트리의 표현법을 쓰고 특징을 적으시오.
    배열 표현법과 링크 표현법
    배열 표현법은 포화이진트리나 완전이진트리에 가장 적합
    링크 표현법은 좌우 링크가 반드시 구별되어야 함
----------------------------------------------------------------------------------------------------
### 이진트리 순회의 종류와 특징을 쓰시오.
    전위순회 (preorder traversal)  : VLR
        전위순회는 루트 먼저 방문 -> 왼쪽 서브 트리 방문 -> 오른쪽 서브 트리 방문
    중위순회 (inorder traversal)   : LVR
        중위순회는 왼쪽 서브 트리 먼저 방문 -> 루트 노드 방문 -> 오른쪽 서브 트리 방문
    후위순회 (postorder traversal) : LRV
        후위순회는 왼쪽 서브 트리 먼저 방문 -> 오른쪽 서브 트리 방문 -> 루트 노드 방문
    레벨순회 (level order)
        레벨순회는 각 노드를 레벨 순으로 검사, 큐가 사용 된다.
        방법은 루트 노드를 큐에 입력 -> 루트 노드 삭제 -> 루트 노드의 왼쪽 자식, 오른쪽 자식 순서대로 입력 -> 큐 맨 앞에 있는 왼쪽 자식의 자식들을 큐에 입력 -> ~~~
        노드를 삭제하면 그 노드의 있는 자식들을 차례대로 입력한다.
        큐를 이용하면 먼저 들어간 노드가 먼저 빠져나가서 위에서 부터 차례대로 왼쪽 -> 오른쪽 방향으로 레벨 순회가 시작 된다.
----------------------------------------------------------------------------------------------------
### 힙의 특징은 무엇인지 쓰고 종류는 무엇인지 쓰시오.
    힙은 여러 개의 값들 중에서 가장 큰(또는 작은) 값을 빠르게 찾아내도록 만들어진 자료구조이고 이진트리 구조를 갖는다.
    힙의 순서 특성과 완전이진트리라는 형태적 특성이 유지되어야 함
    최대 힙과 최소 힙이 있다.
    최대 힙은 부모 노드의 키가 자식 노드의 키보다 크거나 같다. 내림차순으로 나열된다.
    최소 힙은 부모 노드의 키가 자식 노드의 키보다 작거나 같다. 오름차순으로 나열된다.
    장점은 최악의 경우에도 시간 복잡도가 O(nlog2n)이다.
----------------------------------------------------------------------------------------------------
### 주어진 리스트로 maxheap과 minheap을 이진트리 형태로 그리시오
    힙에서 삽입은 제일 마지막 노드에 삽입되고 최대, 최소 힙 조건에 안맞을시 sift-up을 진행한다
    힙에서 삭제는 루트 노드를 삭제하고 마지막 노드를 루트 노드로 올려서 최대, 최소 힙 조건에 맞게 sift-down을 진행한다.
    올리는걸 시프트업 또는 업힙이라고 부름, 내리는걸 시프트다운 또는 다운힙이라고 부름
    조건에 맞게 서로 교환하게 된다.
----------------------------------------------------------------------------------------------------